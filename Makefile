...
test-e2e:
	...
	chmod 644 test/e2e/config/cypress.json
	...
	docker run --rm --user 0 --volume `pwd`/test/e2e/config/cypress.json:/cypress.json --volume `pwd`/test/e2e/cypress:/cypress --volume `pwd`/results:/results --network calc-test-e2e cypress/included:4.9.0 --browser chrome || true
