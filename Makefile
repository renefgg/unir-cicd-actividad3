...
test-e2e:
	...
	docker run --rm --volume `pwd`/test/e2e/config:/config --volume `pwd`/test/e2e/cypress:/cypress --volume `pwd`/results:/results --network calc-test-e2e cypress/included:4.9.0 --config-file /config/cypress.json --browser chrome || true
