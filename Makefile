...
test-e2e:
	...
	docker run --rm --volume `pwd`/test/e2e/config/cypress.json:/config/cypress.json --volume `pwd`/test/e2e/cypress:/cypress --volume `pwd`/results:/results -w /cypress --network calc-test-e2e cypress/included:4.9.0 --config-file /config/cypress.json --browser chrome || true
